...
# Copy initialization script from builder stage and make it executable
COPY --from=builder --chown=aabot:nodejs /app/docker-init.sh ./docker-init.sh
...
CMD ["sh", "-c", "ls -la && pwd && ./docker-init.sh"]